...
from mjlab.tasks.tracking_localobs.mdp.observations import (
...
def unitree_g1_flat_tracking_env_cfg(
  ...
  policy_terms = cfg.observations["policy"].terms.copy()
  for key in ["joint_pos", "joint_vel"]:
      policy_terms.pop(key, None)

  critic_terms = cfg.observations["critic"].terms.copy()
  for key in ["joint_pos", "joint_vel"]:
      critic_terms.pop(key, None)


  policy_terms["body_lin_vel_b"] = ObservationTermCfg(
    func=robot_body_lin_vel_b,
    params={"command_name": "motion"},
  )

  policy_terms["body_ang_vel_b"] = ObservationTermCfg(
    func=robot_body_ang_vel_b,
    params={"command_name": "motion"},
  )
